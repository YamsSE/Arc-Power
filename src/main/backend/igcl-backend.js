...
export class IgclBackend {
   ...
   */
  async _displayOutputsOf(deviceId) {
    if (this._displayHandles.has(deviceId)) return this._displayHandles.get(deviceId);
    const lib = this._libOrThrow();
    const dev = await this._device(deviceId);
    const handles = [];
   ...
   * @param {number} index the output index (the stable display id)
   * @param {object} handle the display handle
   * @returns {Promise<object>} the canonical display shape
   */
  async _readDisplayOutput(index, handle) {
    const lib = this._libOrThrow();
    const d = {
      id: index,
      name: null,
  ...
  async getDisplaySettings(deviceId) {
    try {
      ...
      const handles = await this._displayOutputsOf(deviceId);
      const displays = [];
      for (let i = 0; i < handles.length; i++) {
        const d = await this._readDisplayOutput(i, handles[i]);
   ...
   * @param {number} deviceId
   * @param {number} displayId the display-output index (getDisplaySettings id)
   * @param {import('./backend.interface.js').DisplaySettings} patch
   * @returns {Promise<import('./backend.interface.js').ApplyResult>}
   */
  async setDisplaySettings(deviceId, displayId, patch = {}) {
    const lib = this._libOrThrow();
    const dev = await this._device(deviceId);
    const result = { ok: true, perControl: {} };
    const fail = (control, errorCode, message) => {
    ...
    const handles = await this._displayOutputsOf(deviceId);
    const handle = handles[displayId];
    if (!handle) {
      for (const c of controls) fail(c, 'unsupported', `unknown display id ${displayId}`);
      return result;
    }
          ...
          if (setResult !== CTL_RESULT.SUCCESS) {
            ...
            if (getResult !== CTL_RESULT.SUCCESS) {
              ...
              if (wf.currentUnavailable) {
                ...
                readBackEqual = curModel === patch.wireFormat.model;
                message = readBackEqual
                  ? undefined
                  : `read-back ${curModel ?? 'unknown'} != requested ${patch.wireFormat.model} - the wire-format surface is read-only in effect on this driver build`;
              } else if (wf.current) {
