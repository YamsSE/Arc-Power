...
test('resolveLogonExecPath: packaged portable parent (Arc-Power-*.exe) wins over the temp extraction', async () => {
  ...
  const fakeExec = async (cmd, args) => {
    ...
    return { stdout: 'C:\\Users\\yams\\Downloads\\Arc-Power-1.0.0.exe' };
  };
  ...
  assert.equal(path, 'C:\\Users\\yams\\Downloads\\Arc-Power-1.0.0.exe');
...
test('createStartup: the logonExecPath override drives the Run value (portable story)', async () => {
  ...
  const startup = createStartup({ execFile: fakeExec, logonExecPath: 'C:\\Users\\yams\\Downloads\\Arc-Power-1.0.0.exe' });
  await startup.set(true);
  const add = calls.find((c) => c.includes(' add '));
  assert.ok(add && add.includes('"C:\\Users\\yams\\Downloads\\Arc-Power-1.0.0.exe"'), `add used the logon path: ${add}`);
});
