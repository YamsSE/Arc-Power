...
function osControllerOf(controller) {
  ...
  return {
    ...
    sharedMemoryBytes: Number.isInteger(controller.sharedMemoryBytes) && controller.sharedMemoryBytes > 0 ? controller.sharedMemoryBytes : null,
    sharedMemorySource: typeof controller.sharedMemorySource === 'string' ? controller.sharedMemorySource : null,
...
export function buildGpuInventory({ backendDevices = [], videoControllers = [], backendKind = 'igcl' } = {}) {
  ...
  for (let backendIndex = 0; backendIndex < igcl.length; backendIndex += 1) {
    ...
    rows.push({
      ...
      sharedMemoryBytes: device?.sharedMemoryBytes ?? controller?.sharedMemoryBytes ?? null,
      ...
      osController: osControllerOf(controller ? {
        ...controller,
        sharedMemoryBytes: controller.sharedMemoryBytes ?? device?.sharedMemoryBytes,
        sharedMemorySource: controller.sharedMemorySource ?? device?.sharedMemorySource,
      } : null),
  ...
  controllers.forEach((controller, index) => {
    ...
    rows.push({
      ...
      sharedMemoryBytes: controller?.sharedMemoryBytes ?? null,
...
export function createUnifiedGpuBackend({ backend, sysinfo, videoControllers = null, deferInitialSysinfo = false } = {}) {
  ...
  const wrapper = {
    ...
    async getGameProfileCapabilities(id, exePath) {
      ...
      return backend.getGameProfileCapabilities?.(d.backendId, exePath) ?? { enduranceGaming: false, xeFg: false, xeFgOptions: [], reason: 'Game Profile driver capabilities are unavailable.', xeFgReason: 'Game Profile driver capabilities are unavailable.' };
    },
