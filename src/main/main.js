...
let trayRef = null;
async function setupTray({ getWindow, backend, store, oldIgcl, applyRunner, sysmanPowerLimits = null, createTrayImpl = createTray, iconPath = null }) {
  ...
  const tray = createTrayImpl({ tray: Tray, nativeImage, Menu, template: await menuTemplate(), iconPath });
  trayRef = tray;
...
async function main() {
  ...
  if (holdMainWindowForSplash) {
    ...
    const revealMainWindow = () => {
      ...
      if (startMinimizedAtBoot || win.isDestroyed()) return;
      try {
        win.show();
        win.focus();
      } catch { /* best effort if the window closed during boot */ }
    };
    win.webContents.on('console-message', (event) => {
      const message = typeof event.message === 'string' ? event.message : '';
      if (message.includes('renderer:boot-complete') || message.includes('[renderer] boot complete')) {
        revealMainWindow();
      }
    });
    win.webContents.on('did-fail-load', revealMainWindow);
    revealTimeout = setTimeout(revealMainWindow, 30000);
  }
