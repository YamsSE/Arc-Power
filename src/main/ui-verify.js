...
export async function runUiVerify(win, backend, store, getTrayRebuilds = () => 0, getFpsPolls = () => 0, getWindowOpCounts = () => ({ minimize: 0, maximizeToggle: 0, close: 0 }), getOpenExternalCount = () => 0, getTrayProbe = () => ({ builds: 0, toggleHandler: null, applyHandler: null, doubleClickHandler: null }), sysmanPowerLimits = null, getAppClearCacheRestarts = () => 0) {
  ...
  // M176: the 1.1.6 release pins the titlebar version surface.
  if (!(await waitFor(win, `(document.querySelector('#titlebar-version')?.textContent ?? '').trim() === '1.1.6'`))) {
    fail(`header version line is '${await js(`document.querySelector('#titlebar-version')?.textContent ?? ''`)}' (expected '1.1.6')`);
  }
  ...
  if (!(await waitFor(win, `(() => {
    ...
    return title.trim() === 'GPU 1' && !!gpuKv && (gpuKv.textContent ?? '').trim().length > 0;
  })()`, 8000))) {
  ...
  if (process.env.RID_MOCK_MULTI_DEVICE === '1' && process.env.RID_MOCK_MULTI_ARC === '1' && process.env.RID_MOCK_LEGACY_SELECTOR_VERIFY === '1') {
  ...
  } else if (process.env.RID_MOCK_MULTI_DEVICE === '1' && process.env.RID_MOCK_LEGACY_SELECTOR_VERIFY === '1' && process.env.RID_MOCK_ENDURANCE !== '1') {
    ...
    // M152: Dashboard is an all-adapter inventory in every multi-GPU setup.
    // Focus selection remains available on Tuning/Graphics, while Dashboard
    // keeps one physical card and one live telemetry lane per adapter.
    const devices = await js(`window.arcPower.listDevices()`);
    if (!Array.isArray(devices) || devices.length < 2) {
      fail(`M152: multi-GPU fixture exposed ${JSON.stringify(devices)} (expected at least two adapters)`);
    }
    const focused = await js(`window.arcPower.deviceGet()`);
    const focusedDevice = devices.find((d) => d.id === focused?.deviceId);
    if (!focusedDevice || focusedDevice.integrated === true || focusedDevice.synthetic === true) {
      fail(`M152: startup focus is not a physical dGPU: ${JSON.stringify({ focused, focusedDevice })}`);
    }
    if (await js(`!!document.querySelector('.card-grid .device-select')`)) {
      fail('M152: Dashboard still renders a GPU selector in multi-GPU mode');
    }
    const dashboardCards = await js(`JSON.stringify(Array.from(document.querySelectorAll('.card-grid .device-card[data-device-key]')).map((card) => ({ title: card.querySelector('.card-title')?.textContent?.trim() ?? '', key: card.dataset.deviceKey ?? '', rebar: card.querySelector('.rebar-pill')?.textContent?.trim() ?? '' })))`);
    const parsedCards = JSON.parse(dashboardCards);
    if (parsedCards.length !== devices.length || parsedCards.map((card) => card.title).join('|') !== devices.map((_, index) => `GPU ${index + 1}`).join('|')) {
      fail(`M152: Dashboard GPU cards are ${dashboardCards} (expected one numbered card per adapter)`);
    }
    const pulseLanes = await js(`document.querySelectorAll('.dashboard-pulse-lane').length`);
    ...
    const other = devices.find((d) => d.id !== focused.deviceId) ?? devices[1];
    await js(`(() => { const s = document.querySelector('${tuningSelector}'); s.value = ${JSON.stringify(String(other.id))}; s.dispatchEvent(new Event('change', { bubbles: true })); })()`);
    if (!(await waitFor(win, `window.arcPower.deviceGet().then((d) => d.deviceId === ${other.id})`, 8000))) fail(`M152: Tuning could not switch focus to adapter ${other.id}`);
    await js(`location.hash = '#/dashboard'`);
    if (!(await waitFor(win, `document.querySelectorAll('.card-grid .device-card[data-device-key]').length === ${devices.length} && !document.querySelector('.card-grid .device-select')`, 8000))) fail('M152: switching focused GPU changed Dashboard into a selector view or dropped a card');
    await js(`location.hash = '#/tuning'`);
    await sleep(200);
    await js(`(() => { const s = document.querySelector('${tuningSelector}'); s.value = ${JSON.stringify(String(focused.deviceId))}; s.dispatchEvent(new Event('change', { bubbles: true })); })()`);
    if (!(await waitFor(win, `window.arcPower.deviceGet().then((d) => d.deviceId === ${focused.deviceId})`, 8000))) fail('M152: Tuning could not restore the startup dGPU focus');
    step('m152-multi-gpu-dashboard', `M152: generic multi-GPU Dashboard renders ${devices.length} physical GPU cards and telemetry lanes, has no Dashboard selector, starts focused on dGPU '${focusedDevice.name}', and Tuning can switch/restore focus independently`);
  } else {
...
// restore-Offset click DIES with the Offset/Clock toggle - the M17e
  ...
  if (process.env.RID_MOCK_GAME_SCAN === '1') {
    // The scan is now owned by the catalog header, so this verifier exercises
    // that installed-game flow directly rather than the removed association
    // modal.
    await js(`location.hash = '#/dashboard'`);
    await sleep(150);
    await js(`location.hash = '#/profiles'`);
...
// M176: the 1.1.6 stable bump - Settings displays 'Arc Power Ver. 1.1.6'.
if (!(await waitFor(win, `(document.querySelector('.settings-version')?.textContent ?? '').trim() === 'Arc Power Ver. 1.1.6'`))) {
fail(`M4-D: the Settings version row is '${await js(`document.querySelector('.settings-version')?.textContent ?? ''`)}' (expected 'Arc Power Ver. 1.1.6')`);
  }
...
step('m4d-settings-roundtrips', 'Settings: Close to tray / Start minimized round trips persisted true/false via profiles-settings-save; Log to file is intentionally absent here; version row 1.1.6');
...
export async function runDisplayVerify(win, backend) {
  ...
  for (const expected of ['Scaling Mode', 'Scaling Method', 'Variable Refresh Rate Mode', 'Variable Refresh Rate', 'Quantization Range', 'Graphics Adapter', 'Display Connection', 'HDCP Support', '4K Support', 'HDR Support']) {
    if (!labels.includes(expected)) fail(`D1: Display view is missing the '${expected}' row`);
  }
  for (const hidden of ['Hue', 'Saturation', 'Brightness', 'Contrast', 'Color Depth', 'Color Format']) {
    if (labels.includes(hidden)) fail(`D1: temporarily hidden Display color control '${hidden}' is still rendered`);
  }
...
export async function runGraphicsVerify(win, backend) {
  ...
  if (process.env.RID_MOCK_MULTI_DEVICE === '1' && process.env.RID_MOCK_MULTI_ARC !== '1') {
    ...
    // Dashboard is an all-adapter inventory now. Use the focused-GPU selector
    // on Tuning to choose the iGPU before visiting Graphics.
    await js(`location.hash = '#/tuning'`);
    await sleep(200);
...
export async function runFeaturesetVerify(win, fsId, backend = null) {
  ...
  if (noOc) {
    ...
    // is NESTED inside the freq card now). The b580 watt-unit PL card is
    ...
    if (fsId === 'b580') {
      ...
      // scalar card stack, not a separate Advanced section. The card uses
      // whole-MHz user-facing values while the backend keeps its canonical
  ...
  // --- b580 watt-unit apply round trip --------------------------------------
  if (fsId === 'b580') {
    ...
    if ((await js(`window.arcPower.waiverGet(0)`)).accepted !== true) {
      await setSlider(228);
      await clearToasts();
      if (!(await clickApply())) fail('floating Apply did not appear for the b580 watt apply');
    ...
    if (Math.abs(applied.powerLimitW - 120) > 1e-6) fail(`b580 watt apply did not stick: ${applied.powerLimitW}`);
    ...
    // watt-unit PL card remains alongside it; Battlemage has no Stock/Advanced
    // mode card at all.
    if (await js(`!!document.querySelector('.oc-mode-col-mode')`)) {
      fail('M4J (D): the b580 Stock/Advanced toggle is visible (Battlemage must hide it)');
    }
...
//       rows - M18: every value-unit pair is GLUED, never '50 W'; M19: the
//       rows carry the divider-aligned padded labels - every value starts
//       at maxLabelLen + 2 ch, right of the divider); the
 ...
 * @param {{ getWindow: () => import('electron').BrowserWindow | null, getState: () => object, setRecordingState: (state: object|null) => void }|null} [recordingStatusPillHandle]
 *   M143: the sibling top-right recording status overlay
 */
export async function runOverlayVerify(win, overlayHandle, store, hotkeyProbe, getFpsPolls = () => 0, recordingStatusPillHandle = null) {
  const log = (s) => console.log(`[ui-verify] ${s}`);
  const steps = [];
  const step = (n, msg) => {
    steps.push(`[${n}] ${msg}`);
  ...
  const statusPillWin = recordingStatusPillHandle?.getWindow?.() ?? null;
  ...
  // M144: the recording indicator is a sibling transparent window pinned to
  // the active display's top-right corner. Exercise the live state path with
  // synthetic engine envelopes so the verifier never needs a native capture
  // runtime or a real recording file.
  if (!statusPillWin || statusPillWin.isDestroyed()) {
    fail('M144: the recording status pill window does not exist in the overlay variant');
  }
  ...
  if (!pillBounds || pillBounds.x + pillBounds.width > displayBounds.x + displayBounds.width || pillBounds.y < displayBounds.y) {
    fail(`M144: the recording status pill is not inside the display's top-right corner (bounds=${JSON.stringify(pillBounds)})`);
  }
  recordingStatusPillHandle.setRecordingState({
    running: true,
    mode: 'replay',
  ...
  // the stats on and back). M19/M19b: the 'FPS' label is padded to the 4ch
  ...
  // the row label reads 'RAM'; M18: the unit is GLUED; M19/M19b: the label
  // is padded to the 4ch column + the TWO-space separator - 'RAM   12.4GB').
  // M16: the VRAM row
  ...
  // (m18-divider) M18/M19b: the header divider - ONE absolutely-positioned
  // 1px line behind the SIX labeled rows (FPS/CPU/RAM/GPU/VRAM/API),
  // spanning the fps->api block (M19b: the API row JOINED the divider
  // column - the line extends to its bottom). The expected x is derived
  // INDEPENDENTLY - never the production calc string: the root padding via
  // getComputedStyle + the max label length across the SIX rendered rows
  // (the api row's padded 'API' label contributes its 3ch - it never
  // widens the column) + a measured per-char width in the divider's own
  // font. The divider carries the overlay color with NO text-shadow
  // (deliberate).
  const dividerPins = await ojs(`(() => {
    ...
    probe.remove();
    const expectedX = parseFloat(getComputedStyle(root).paddingLeft) + labelW + 0.75 * charW;
  ...
  // puts every row's VALUE at `maxLabelLen + 2 ch` - 2ch RIGHT of the
  // divider (the divider sits at maxLabelLen + 0.75ch). Derived inside the
  // page: the value start is the left edge of a Range over the row's text
  // node starting at char (maxLabelLen + 2) - the exact offset the ONE
  // rule puts the first value at (maxLabelLen = the renderer's
  // --overlay-label-w CSS var - the per-render source of truth set from
  // the TRUE max of the SIX labels; never a DOM-text tokenization - the
  // space-containing chip labels like 'i7 14700K' would tokenize to 4
  // chars instead of 9). Assert: every value start is RIGHT of the
  // divider's x AND the value starts are EQUAL (the aligned column) -
  // over the rows WITH a text node only (an empty row - the api row
  // without RID_MOCK_API - has no value start: '' has no firstChild).
  const dividerAlignPin = () => `(() => {
    const divider = document.getElementById('overlay-divider');
    if (!divider) return { ok: false, why: 'missing-divider' };
    const ids = ['overlay-fps', 'overlay-cpu', 'overlay-memory', 'overlay-gpu', 'overlay-vram', 'overlay-api'];
    const els = [
  ...
  const alignPins = await ojs(dividerAlignPin());
  if (!alignPins.ok) {
    fail(`M19: the overlay divider-alignment pins failed (${alignPins.why}) - every labeled row's value must start RIGHT of the divider and the value starts must be EQUAL`);
  }
  ...
  // (f2a-m144) M144: the pill preference belongs to the Recording tab, not
  // the Monitoring/Overlay settings surface. Keep the synthetic active video
  // state alive while flipping it so persistence and immediate hide/show are
  // verified through the user-facing location.
  if (await js(`!!document.querySelector('.settings-checkbox[data-setting="overlayRecordingPill"]')`)) {
  ...
  await js(`location.hash = '#/monitoring'`);
  if (!(await waitFor(win, `!!document.querySelector('#overlay-settings-root')`, 8000))) {
    fail('M144: returning to Monitoring did not restore the Overlay settings surface');
  }
  ...
  // (m19-divider-alignment-wide) M19/M19b: under the widened 9ch column the
  // ONE rule still holds - every value starts right of the WIDER divider
  // and the value starts stay equal (the same var-derived pin as the
  // stock pass; --overlay-label-w 9ch -> maxLen 9 -> offset 11).
  const alignWide = await ojs(dividerAlignPin());
  if (!alignWide.ok) {
  ...
  if (mockApi) {
    ...
    step('m19b-api-wide', "the API row under the chip-names wide column stays EMPTY (no RID_MOCK_API - the vanish rule)");
  }
  await js(`(() => { const b = document.querySelector('.settings-checkbox[data-setting="overlayChipNames"]'); if (b) b.click(); })()`);
  ...
  // percentile stats ON (each re-checks at its end), so the pin is
  // avg01Pin, never the boot-plain fpsPin (the M19/M19b padded shapes
  // apply to both).
  if (mockApi) {
