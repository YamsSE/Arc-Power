...
export class RecordingStore {
      ...
      const walk = (dir, depth = 0) => {
        ...
        for (const entry of entries) {
          ...
          if (stat.isDirectory()) {
            ...
            found.push({
              // A scan refreshes file facts, but it must retain metadata that
              // was derived when the clip was created. In particular, APM
              // samples and availability were previously dropped here, so a
              // restart or library refresh made the tracker disappear.
              ...(old ?? {}),
  ...
  async recordClip({ relativePath, fileName, createdAt = new Date().toISOString(), markerSummaries = [], apmSamples, apmAverage, apmPeak, apmAvailable, editorVersion = RECORDING_EDITOR_VERSION } = {}) {
    return this._enqueueMutation(() => {
      const { data: current } = this._readData();
      ...
      const clip = normalizeRecordingClip({
        ...
        markerSummaries,
        ...(apmSamples !== undefined ? { apmSamples } : {}),
        ...(Number.isFinite(apmAverage) ? { apmAverage } : {}),
        ...(Number.isFinite(apmPeak) ? { apmPeak } : {}),
        ...(typeof apmAvailable === 'boolean' ? { apmAvailable } : {}),
        editorVersion,
      });
